<template>
  <div class="videoinputBox" :style="{width:width+'px',height:height+'px'}">
    <div class="header">
      <span class="back">
    ...
    doComment() {
      ...
      $(".inputBtn").on("click", function() {
        ...
        me.axios
          ...
          .then(function(response) {
            //更新文章的评论数量
            let  conId = localStorage.getItem("videoid");
            
            me.axios
              .post(config.server + config.port.video.updateComment, {
                id: conId,
              })
              .then(function(response) {
                me.$router.go(-1);
